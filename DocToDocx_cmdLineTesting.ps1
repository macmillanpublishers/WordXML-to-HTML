...
echo "Converting $path to .docx from $fileType..."
write-host $path
$doc = $word.documents.open($_.FullName)
$wdFormatDocx = 16  # XML format is 19
...
$ChkFile = $($currVol + "\bookmaker_tmp\" + $filename + "\" + $filename + ".docx")
$FileExists = Test-Path $ChkFile 
If ($FileExists -eq $True) {LogWrite "pass: inputFile.docx exists in $($currVol)\bookmaker_tmp\inputFile\."}
Else {LogWrite "FAIL: inputFile.docx exists in $($currVol)\bookmaker_tmp\inputFile\."}

#if input file was not an .doc the orig should still exist in bookmaker_tmp\inputFile\
$fileExt = $filenameSplit.SubString(1, $filenameSplit.LastIndexOf('.'))
$DocExists = Test-Path $folderpath -include *.doc, *.docx 
If (($fileExt -ne "xml") -And ($DocExists -eq $True)) {"pass: if orig input file was .docx, it still exists in $($currVol)\bookmaker_tmp\inputFile\."}
Else {LogWrite "FAIL: if orig input file .docx, it still exists in $($currVol)\bookmaker_tmp\inputFile\."}
